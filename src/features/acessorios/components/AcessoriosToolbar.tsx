...
export function AcessoriosToolbar({ categories, onSearch, onFilter, labels }: AcessoriosToolbarProps) {
  ...
  return (
    <div className="sticky top-[116px] z-40 bg-white border-b border-black">
      <div className="container mx-auto px-6 flex items-center justify-between gap-6 py-3">
        <div className="flex items-center gap-4">
          <button className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-neutral-500 hover:text-brand-red-500 transition-colors">
          ...
          <div className="hidden items-center gap-6 lg:flex">
            {categories.map((cat) => (
              <button 
                ...
                className="text-[10px] font-bold uppercase tracking-widest text-neutral-500 transition-colors hover:text-black"
              >
                {cat.label}
              </button>
            ))}
